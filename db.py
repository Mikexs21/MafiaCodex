"""Async SQLite access layer for Mafia bot."""
...
async def init_db() -> aiosqlite.Connection:
    """Initialize connection and schema lazily."""
    global _DB
    if _DB is None:
        db_path = Path(config.SQLITE_DB_PATH)
        _DB = await aiosqlite.connect(
            db_path, isolation_level=config.SQLITE_ISOLATION_LEVEL
        )
...
async def _setup_schema() -> None:
    ...
    await _DB.executescript(
        ...
        CREATE TABLE IF NOT EXISTS purchases (
            ...
            effect_consumed_until INTEGER DEFAULT 0,
...
async def update_points(user_id: int, delta: int, win: bool = False) -> None:
    await init_db()
    async with _async_lock:
        await _DB.execute(
            "UPDATE users SET total_points = total_points + ?, total_games = total_games + 1, total_wins = total_wins + ?"
            " WHERE id=?",
...
async def update_game_player_status(game_id: int, name: str, alive: bool) -> None:
    ...
    async with _async_lock:
        await _DB.execute(
            "UPDATE game_players SET is_alive=? WHERE game_id=? AND name=?",
            (int(alive), game_id, name),
        )
        await _DB.commit()
...
async def list_shop_items() -> List[Dict[str, Any]]:
    ...
    async with _async_lock:
        cursor = await _DB.execute(
            "SELECT id, code, name_uk, description_uk, cost_points, effect_type, effect_value FROM shop_items"
        )
...
async def purchase_item(user_id: int, item_code: str) -> str:
    ...
    async with _async_lock:
        ...
        await cursor.close()
        if not item:
            return "Нема такого товару, ти шо."
        ...
        if not row or row[0] < cost:
            return "Не вистачає очок. Працюй ще."
        await _DB.execute(
        ...
        await _DB.commit()
        return "Купівля успішна. Бонус додано."
...
async def get_user_buffs(user_id: int) -> List[Dict[str, Any]]:
    ...
    async with _async_lock:
        cursor = await _DB.execute(
            "SELECT id, buff_type, remaining_games, parameters FROM user_buffs WHERE user_id=?",
            (user_id,),
        )
        rows = await cursor.fetchall()
        await cursor.close()
...
async def consume_active_role_buff(user_id: int) -> bool:
    ...
    async with _async_lock:
        cursor = await _DB.execute(
            "SELECT id, remaining_games FROM user_buffs WHERE user_id=? AND buff_type='active_role' AND remaining_games>0"
        ...
        if not row:
            return False
        buff_id, _remaining = row
        await _DB.execute(
            "UPDATE user_buffs SET remaining_games = remaining_games - 1 WHERE id=?",
            (buff_id,),
        )
        await _DB.commit()
        return True
